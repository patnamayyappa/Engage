...
module CampusManagement.cmc_dommaster {
  declare var CampusManagement: any;
  declare let fallBackUser: Xrm.Controls.LookupControl;
    var _previousMemberType: string = null,
        _runAssignmentAttr,
        _attributeNameToBeAssignedAttr,
        _otherUserLookupAttr,
        _otherUserLookupCtrl;
    ...
    export enum AttributeNameToBeAssignedType {
        RecordOwner = 175490000,
        OtherUserLookup = 175490001
    }
    ...
    export function onLoad(executionContext) {
      ...
      fallBackUser.addPreSearch(addCustomLookupFilterForfallBackUser);
        ...
        if (formContext.getAttribute("cmc_marketinglistid")) {
            let previousMarketingList = formContext.getAttribute("cmc_marketinglistid").getValue();
    ...
    function runValidations(formContext) {
            ...
            retrieveRelatedRecords(formContext.data.entity.getId())
                ...
                function (relatedRecords) {
                    ...
                    for (i = 0; i < l; i++) {
                        if (!relatedRecords.entities[i]["eo.cmc_attributeschema"] || flags[relatedRecords.entities[i]["eo.cmc_attributeschema"]]) continue;
                    ...
                    flags = [];
                    for (i = 0; i < l; i++) {
                        if (!relatedRecords.entities[i]["ddl.cmc_attributeschema"] || flags[relatedRecords.entities[i]["ddl.cmc_attributeschema"]]) continue;
                    ...
                    attributeValidation(formContext, executionOrderAttributes, definitionLogicAttributes,
                        translations["DefinitionLogicAttributeMismatch_SingleError"], translations["DefinitionLogicAttributeMismatch_MultipleErrors"]);
                    attributeValidation(formContext, definitionLogicAttributes, executionOrderAttributes,
                        translations["DomExecutionOrderAttributeMismatch_SingleError"], translations["DomExecutionOrderAttributeMismatch_MultipleErrors"]);
                },
                function (error) {
                    console.log(error);
                }
    ...
    function filterRunAssignmentForEntity(memberType, optionValue) {
        let runAssignmentForEntity = _runAssignmentAttr.controls,
            runAssignmentForEntityOptions;
        ...
        if (memberType) {
            ...
            runAssignmentForEntity.forEach(function (control) {
                ...
                for (var i = 0; i < runAssignmentForEntityOptions.length; i++) {
                    let runAssignmentForEntityOption = runAssignmentForEntityOptions[i];
                    let option = _runAssignmentAttr.getOption(runAssignmentForEntityOption);
    ...
    function onMarketingListChange(executionContext) {
        var formContext: Xrm.FormContext = executionContext.getFormContext();
        let marketingList = formContext.getAttribute("cmc_marketinglistid").getValue();

        if (marketingList && marketingList.length > 0) {
            let option: string = null;
            let marketingListId = marketingList[0].id;
            getMemberType(marketingListId).then(function (memberType) { 
                ...
                if (_previousMemberType !== memberType) {
                    ...
                    if (memberType != "2") {
                        option = _config[memberType][0];
                    }
                    filterRunAssignmentForEntity(memberType, option);
                }
