...
module CampusManagement.StaffCalendar {
  ...
  export function locationOnChange(e) {
  ...
  export function departmentOnChange(e) {
  ...
  export function userOnChange(e) {
  ...
  function initUserSelect() {
    var $departmentSelect = <JQuery<HTMLElement>>$('#departmentSelect'),
      departmentId = $departmentSelect.val(),
      $locationSelect = <JQuery<HTMLElement>>$('#locationSelect'),
      accountId = $locationSelect.val(), // used Jquery val event
  ...
  function retrieveLocations() {
    var fetchXml = [
      ...
      '<condition attribute="mshied_accounttype" operator="eq" value="494280000" />',  // Account Type == Campus
      '</filter>',
  ...
  function getFacultyMemberAvailability() {
    var $selectedUser = <JQuery<HTMLElement>>$("#userIdSelect option:selected"),
      userId = $selectedUser.length > 0 ? $selectedUser.val() : null,
      $selectedAccount = <JQuery<HTMLElement>>$("#locationSelect option:selected"),
      accountId = $selectedAccount.length > 0 ? $selectedAccount.val() : null,
      input = {};
  ...
  function grayOutUnavailableSlots() {
    var allSlots: JQuery<Element> = $(".k-scheduler-table td"),
      $selectedUser = <JQuery<HTMLElement>>$("#userIdSelect option:selected"),
      userId = $selectedUser.length > 0 ? $selectedUser.val() : null,
      rangesOnDate;
    ...
    allSlots.each(function (i, item) {
      var slot = _scheduler.slotByElement(item),
  ...
  function toggleSlot(slot, toggle, showAddIcon = false) {
    if (toggle) {
      $(slot).removeClass("unavailable");

      if (showAddIcon) {
        if ($(slot).find("i").length <= 0) {
          let icon = $("<i>").addClass('fa fa-calendar-plus-o').text(" "+localizedStrings.bookAppointment).css({ "color": "green", "font-size": "small" }).click(e => {
  ...
  function compareSlots(a, b) {
    var slotADate = _scheduler.slotByElement(a).startDate.getDate(),
      slotBDate = _scheduler.slotByElement(b).startDate.getDate();
  ...
  function createAppointment(e) {
    var selectedUser = <JQuery<HTMLElement>>$("#userIdSelect option:selected"),
      userId = selectedUser.length > 0 ? $("#userIdSelect option:selected").val() : null,
      start = e.event.start,
      end = e.event.end,
    ...
    SonomaCmc.WebAPI.post('cmc_CreateStaffAppointment', input)
      .then(
        function (result) {
          var appt = JSON.parse(result.StaffAppointmentsJson)[0],
            data = new kendo.data.SchedulerEvent(<kendo.data.SchedulerEventData>{
  ...
  function retrieveUserAppointments() {
    var selectedOption = <JQuery<HTMLElement>>$("#userIdSelect option:selected"),
      userId = selectedOption.length > 0 ? $("#userIdSelect option:selected").val() : null,
      input = { "UserId": userId };
      ...
      .then(
        function (result) {
          ...
          appointments.forEach(function (appt) {
            ...
            let start = new Date(Date.parse(appt.start)),
              end = new Date(Date.parse(appt.end));
            start.setTime(start.getTime() - start.getTimezoneOffset() * 60 * 1000);
            end.setTime(end.getTime() - end.getTimezoneOffset() * 60 * 1000);

            var data = new kendo.data.SchedulerEvent(<kendo.data.SchedulerEventData>{
              id: appt.appointmentId,
              appointmentId: appt.appointmentId,
              userId: userId,
