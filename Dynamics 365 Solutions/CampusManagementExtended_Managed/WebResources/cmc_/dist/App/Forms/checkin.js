    ...
    (function (checkin) {
        function onLoad() {
            ...
            if (Xrm.Page.ui.getFormType() === 1 /* Create */) {
                Xrm.Device.getBarcodeValue().then(function success(qrcode) {
                    Xrm.Page.getAttribute("cmc_scanqrcode").setValue(qrcode);
                    Xrm.Page.getAttribute("cmc_scanqrcode").fireOnChange();
                });
            }
        }
        checkin.onLoad = onLoad;
        function scanQRCode(executionContext) {
            ...
            if (!code.startsWith("sr") && !code.startsWith("er"))
                return;
            getRegistrationInfo(code).then(function success(registrationInfo) {
                updateCheckIn(executionContext, registrationInfo);
            });
        }
        checkin.scanQRCode = scanQRCode;
        function updateCheckIn(executionContext, registrationInfo) {
            ...
            if (registrationInfo.entities.length > 0) {
                ...
                var registrationEntity = registrationInfo.entities[0];
        ...
        function setEventCheckInType(formContext, value) {
            var checkinType = formContext.getAttribute("msevtmgt_checkintype");
            if (checkinType) {
        ...
        function setLookupValue(formContext, lookupFieldId, id, name, entityName, fireOnChange) {
            var lookupField = formContext.getAttribute(lookupFieldId);
            if (lookupField) {
                var lookupValue = [{ id: id, name: name, entityType: entityName }];
                lookupField.setValue(lookupValue);
                if (fireOnChange)
                    lookupField.fireOnChange();
            }
        }
    })(checkin = CampusManagement.checkin || (CampusManagement.checkin = {}));
