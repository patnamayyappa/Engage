    ...
    (function (Engage) {
        ...
        (function (invoice) {
            ...
            function onLoad(executionContext) {
                var formContext = executionContext.getFormContext(), shipTo = formContext.getAttribute('willcall'), applicationId = formContext.getAttribute('cmc_applicationid'), customerId = formContext.getAttribute('customerid'), priceLevelId = formContext.getAttribute('pricelevelid'), disableApplicationControlledFields;
                ...
                _formLoaded = true;
                if (formContext.ui.getFormType() === 1 /* Create */ && shipTo) {
            ...
            function populateContactAndPriceListFromApplication(executionContext, skipClearField) {
                var formContext = executionContext.getFormContext(), applicationId = formContext.getAttribute('cmc_applicationid'), customerId = formContext.getAttribute('customerid'), priceLevelId = formContext.getAttribute('pricelevelid'), loadingText, fetchXml;
                ...
                Xrm.WebApi.retrieveMultipleRecords('cmc_application', '?fetchXml=' + fetchXml).then(function (results) {
                    ...
                    contactId = record['_cmc_contactid_value'] || record['applicationRegistration.cmc_contactid'], contactName = record['_cmc_contactid_value@OData.Community.Display.V1.FormattedValue'] || record['applicationRegistration.cmc_contactid@OData.Community.Display.V1.FormattedValue'], priceListId = record['invoiceDefinition.cmc_pricelistid'], priceListName = record['invoiceDefinition.cmc_pricelistid@OData.Community.Display.V1.FormattedValue'];
                    disableEnableClearSetField(customerId, true, skipClearField, contactId ? { id: contactId, entityType: 'contact', name: contactName } : null);
                    disableEnableClearSetField(priceLevelId, true, skipClearField, priceListId ? { id: priceListId, entityType: 'pricelevel', name: priceListName } : null);
                }, function (error) {
                    Xrm.Utility.closeProgressIndicator();
            ...
            function disableEnableClearSetField(attribute, disabled, skipClearField, value) {
        ...
        })(invoice = Engage.invoice || (Engage.invoice = {}));
