...
(function (CMC) {
    ...
    (function (Engage) {
        ...
        (function (AttributeSelector) {
            var _attribute, _attributeSchema, _domMaster, _status, _removeOptionFromLastSession, _optionOnLastSession, localizedStrings;
            var StateCode;
            (function (StateCode) {
                StateCode[StateCode["Active"] = 0] = "Active";
                StateCode[StateCode["Inactive"] = 1] = "Inactive";
            })(StateCode = AttributeSelector.StateCode || (AttributeSelector.StateCode = {}));
            ;
            ...
            AttributeSelector.executeOnLoad = executeOnLoad;
            ...
            function onLabelClick(event) {
                var $label = $(event.currentTarget), $dropdown = $('#' + $label.data('dropdown-label-for'));
            ...
            function onDropdownSet(event) {
                var $option = $(event.currentTarget), $dropdownContainer = $(event.currentTarget).parent('.dropdown'), $label = $('[data-dropdown-label-for=' + $dropdownContainer.attr('id') + ']');
                if ($label.length) {
            ...
            function displayWarningIconOnSave(executionContext) {
                var $dropdownContainer = _$select.parent('dropdown'), $label = $('[data-dropdown-label-for=' + $dropdownContainer.attr('id') + ']');
                if (_$select.val()) {
            ...
            function disableField() {
                if (parent.Xrm.Page.ui.getFormType() !== 3 /* ReadOnly */ &&
                    parent.Xrm.Page.ui.getFormType() !== 4 /* Disabled */ &&
            ...
            function refreshCustomDropdown() {
                ...
                getAttributeNames()
                    ...
                    if (attributeNameInput) {
                        getAttributeDisplayNamesJson(attributeNameInput).then(function success(results) {
                            toggleLoading(false);
                            if (results.length > 0) {
            ...
            function getAttributeNames() {
                var domMasterId = _domMaster.getValue() ? _domMaster.getValue()[0].id : null, attributeKeysStr = '';
                if (!domMasterId) {
                    return new SonomaCmc.Promise.Promise(function (resolve, reject) { return resolve(''); });
                }
                return fetchDomDefinitionLogicAttributesFromDomMaster(domMasterId).then(function success(results) {
            ...
            function getAttributeDisplayNamesJson(input) {
                ...
                return SonomaCmc.WebAPI.post('cmc_buildattributedisplay', {
                    ...
                    .then(function (results) { return JSON.parse(results.AttributeDisplayNamesJson); })
            ...
            function isCurrentAttributeSchemaAnOption() {
            ...
            function getAttributeKeysAsString(domDefinitionLogic) {
                var attributes = [], length = domDefinitionLogic.length, schema;
                // Convert and return attribute names as string delimited by ','
                for (var i = 0; i < length; i++) {
                    schema = domDefinitionLogic[i].cmc_attributeschema;
                    if (!(attributes.some(function (s) { return s === schema; }))) {
        ...
        })(AttributeSelector = Engage.AttributeSelector || (Engage.AttributeSelector = {}));
