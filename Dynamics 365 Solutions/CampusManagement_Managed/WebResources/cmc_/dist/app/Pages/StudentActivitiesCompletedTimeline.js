...
(function (CampusManagement) {
    var StudentActivitiesCompletedTimeline;
    (function (StudentActivitiesCompletedTimeline) {
        var listOfLifecyleName = [], listOfLyfecycleId = [], extendedEntites = [];
        var timeline, localizedStrings, items = new vis.DataSet(), itemDetails = {}, appointmentStatus = {
            "Cancelled": 4
        }, phoneStatus = {
            "Cancelled": 3
        }, emailStatus = {
        ...
        function executeOnLoad() {
            ...
            retrieveTimelineActivities(studentId).then(function (results) {
        ...
        StudentActivitiesCompletedTimeline.executeOnLoad = executeOnLoad;
        ...
        function updateDropDown() {
            ...
            var unSelectedLyfecycleId = listOfLyfecycleId.filter(function (item) { return selectedLyfecycleId.indexOf(item) < 0; });
        ...
        function buildVisDataSet(studentActivityResults) {
            var studentPhonecalls = studentActivityResults[0].entities, studentAppointments = studentActivityResults[1].entities, studentToDos = studentActivityResults[2].entities, studentEmails = studentActivityResults[3].entities, studentEmailsForLifecycles = studentActivityResults[4].entities, studentAppointmentsForLifecycles = studentActivityResults[5].entities, studentPhonecallsForLifecycles = studentActivityResults[6].entities, studentEvents = (studentActivityResults[7].entities ? studentActivityResults[7].entities : []), marketingEmailsSent = studentActivityResults[8], studentChats = (studentActivityResults[9].entities ? studentActivityResults[9].entities : []), detail, i;
                ...
                for (i = 0; i < studentToDos.length; i++) {
                    ...
                    try {
                        items.add({
                            ...
                            start: convertUtcDateToLocalDate((studentToDos[i].statuscode === todoStatus.Complete) ? (studentToDos[i].cmc_completedcanceleddate) : (studentToDos[i]['cmc_duedate@OData.Community.Display.V1.FormattedValue']), "YYYY-MM-DDTHH:mm"),
                            className: 'to-do',
        ...
        function consolidateAppointmentsOnRequiredAttendees(appointments) {
            var consolidatedAppointments = [], activityids = [];
        ...
        function removeDuplicate(requiredattendees) {
            var requiredattendeeArray = requiredattendees.split(',').map(function (string) { return string.trim(); });
            var removeDuplicateRequiredattendee = [];
            for (var requiredattendee in requiredattendeeArray)
                if (removeDuplicateRequiredattendee.indexOf(requiredattendeeArray[requiredattendee]) == -1)
                    removeDuplicateRequiredattendee.push(requiredattendeeArray[requiredattendee]);
        ...
        function prepareMarketingEmails(marketingEmail) {
            ...
            items.add({
                ...
                content: prepareTimelineContentWithIcon(marketingEmail.subject, Constants.StudentTimelineActivity.marketingEmail, ResourseStrings.StudentTimelineActivity.Email, '', ''),
        ...
        function prepareTimelineContentWithIcon(content, iconPath, tooltip, lifecycleName, lifecycleId, status, iconClass) {
            ...
            if (iconPath === "") // cases when no icon path given : ex: incomleted to do activities.
                ...
                return "<img class='vis-item-" + tooltip.replace(" ", "") + "-icon' src='../Images/ContactActivitiesTimeline/" + iconPath + "-32x32.svg' title='" + tooltip + "'></img><span style='margin-right:10px;' title = '" + content + "'> " + content + "</span>" + (status ? "<i class='fa " + iconClass + " vis-item-status " + status + "-icon' aria-hidden='true'></i>" : "");
            else
        ...
        function setAppointmentDetails(appointment) {
            ...
            var scheduledstart = getDateFormattedValue(appointment['scheduledstart@OData.Community.Display.V1.FormattedValue']), scheduledend = getDateFormattedValue(appointment['scheduledend@OData.Community.Display.V1.FormattedValue']);
            var title = [
        ...
        function setToDoDetails(todo) {
            var peekDescription = cleanupActivityDescription(todo.cmc_description), ownerid = todo['_ownerid_value@OData.Community.Display.V1.FormattedValue'];
            var title = [
                '<div class="item-title">',
                todo.cmc_todoname,
                '<div class="item-details">',
        ...
        function setPhonecallDetails(phonecall) {
            var peekDescription = cleanupActivityDescription(phonecall.description), from = getFromNameInActivityParties(phonecall['phonecall_activity_parties']);
            var title = [
                '<div class="item-title">',
                phonecall.subject,
                '<div class="item-details">',
        ...
        function setEmailDetails(email) {
            var peekDescription = cleanupActivityDescription(email.description), from = getFromNameInActivityParties(email['email_activity_parties']);
            var title = [
                '<div class="item-title">',
                email.subject,
                '<div class="item-details">',
        ...
        function cleanupActivityDescription(description) {
            ...
            var regexHTMLTags = /(<head>.+?(?=<\/head>)<\/head>)|(<([^>]+)>)/ig, peekLength = 50;
            if (!description) {
                return 'N/A';
            }
            else {
                var peekDescription = description.replace(regexHTMLTags, ''), overflow = peekDescription.length - peekLength;
                if (overflow > 0) {
        ...
        function retrieveMarketingEmails(studentId) {
            ...
            return verifyMarketingIsInstalled().then(function (isInstalled) {
                ...
                var allInteractions = JSON.parse(response.Data), sendsById = {}, templateId, fetchXml, i, totalLength;
                ...
                fetchXml =
                    "<fetch version=\"1.0\" output-format=\"xml-platform\" mapping=\"logical\" distinct=\"false\">                    <entity name=\"msdyncrm_marketingemail\">                    <attribute name=\"msdyncrm_marketingemailid\" />                    <attribute name=\"msdyncrm_subject\" />                    <attribute name=\"msdyncrm_emailbody\" />                    <attribute name=\"msdyncrm_description\" />                   \n                    <attribute name=\"msdyncrm_fromuser\" />                    <filter>                        <condition attribute=\"msdyncrm_marketingemailid\" operator=\"in\">                         <value>" + Object.keys(sendsById).join('</value><value>') + "</value>                         </condition>                     </filter>                    </entity>                </fetch>";
                return Xrm.WebApi.retrieveMultipleRecords('msdyncrm_marketingemail', '?fetchXml=' + fetchXml).
                    then(function (response) {
                        var marketingEmail, templateSends, finalSends = [], peekDescription, j, marketingEmailsLength;
        ...
        function verifyMarketingIsInstalled() {
            ...
            var retrieveMarketingWorkflow = '<fetch top="1">\
        ...
        var itemClass = null;
        var itemIdPrefix = null;
        function toggleHideItems(event) {
        ...
        StudentActivitiesCompletedTimeline.toggleHideItems = toggleHideItems;
        function toggleLifeCycle(selectedLyfecycleId, unSelectedLyfeCycleId) {
            ...
            var visibleItems = $.map(items._data, function (item, index) {
                if (item.isVisibleInTimeline == true || item.isActivityEnabled == true) {
                    return item;
                }
            });
            ...
            for (var i = 0; i < unSelectedLyfeCycleId.length; i++) {
                ...
                var itemsToHide = $.map(items._data, function (item, index) {
                    if (item.lifeCycleId == trimLfCycleId)
                        return item;
                });
            ...
            for (var i = 0; i < selectedLyfecycleId.length; i++) {
                ...
                $(lifeCycleItems).each(function (index, item) {
                    items.update({ id: item.id, lifeCycleId: trimLfCycleId, className: 'hidden', isLifeCycleEnabled: true });
                });
                var itemsToShow = $.map(visibleItems, function (item, index) {
                    if (item.lifeCycleId == trimLfCycleId)
                        return item;
                });
    ...
    })(StudentActivitiesCompletedTimeline = CampusManagement.StudentActivitiesCompletedTimeline || (CampusManagement.StudentActivitiesCompletedTimeline = {}));
