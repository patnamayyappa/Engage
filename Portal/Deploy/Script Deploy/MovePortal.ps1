...
function ProcessEntityRecords($EntityName, $Records, $SourceMetadata, $TargetMetadata, $GlobalBlacklistedFields, $EntityBlackListedFields) {
    $Null = @(Try {
        ...
        $invalidAttributes = $sourceInvalidAttributes + $targetInvalidAttributes + $GlobalBlacklistedFields

        $blacklistedFields = $EntityBlackListedFields | Where-Object {$_.entity -eq $EntityName} | Select-Object -First 1

        if ($blacklistedFields -ne $null) {
            $invalidAttributes += $blacklistedFields.fields
        }
    ...
    Catch {
        Write-Error -Message ("Error in the processing entity records task: " + $_.Exception.Message);
        throw
    })
    return $Records
}

function GetEntityRecords($CrmConnection, $Entities, $SourceMetadata, $TargetMetadata, $GlobalBlacklistedFields, $EntityBlackListedFields, $entityFilterXml) {
    $Null = @(Try {
            ...
            Foreach ($entity in $Entities) {
                ...
                elseif ([bool]($entityFilterXml.PSObject.Properties.name -match $entity)) {
                    Log "Processing filter for $entity"

                    $condition = $entityFilterXml | select -ExpandProperty $entity;
                ...
                if ($records -and $records.CrmRecords) {
                    $processedRecords = ProcessEntityRecords $entity $records.Get_Item("CrmRecords") $SourceMetadata $TargetMetadata $GlobalBlacklistedFields $EntityBlackListedFields
                    if (!$processedRecords) {
    ...
    try {
        ...
        Foreach ($upsertRecord in  $UpsertRecords) {
            ...
            try {
                ...
                Write-Error -Message ("Failed Upserting " + $upsertRecord.EntityReference.LogicalName + " " + $upsertRecord.EntityReference.Id)
    ...
    try {
        ...
        Foreach ($updateLookup in  $UpdateRecords) {
            ...
            try {
                ...
                Write-Error -Message ("Failed to update lookups " +  $updateLookup.LogicalName + " " + $updateLookup.Id)
                Write-Error -Message ($_.Exception.Message);
            }
        }
    }
...
function UpsertAnnotations($CrmConnection, $AnnotationRecords) {
    try {
        ...
        Foreach ($annotationRecord in $AnnotationRecords) {
            $runningCount++;
            Log "Upsert Annotation $runningCount / $($AnnotationRecords.Count)"
            ...
            try {
                ...
                Write-Error -Message ("Failed to upsert annotation " + $annotationRecord.EntityReference.Id)
                Write-Error -Message ($_.Exception.Message);
            }
        }
    }
...
function AssociateRecords($CrmConnection, $NNRecords, $Metadata) {
    try {
        ...
        Foreach ($nnrecord in $NNRecords) {
            ...
            Catch {
                if ($_.Exception.Message -eq "Cannot insert duplicate key." -or $_.Exception.Message -like "Duplicate Record Found for Entity:*") {
...
try {   
    ...
    Log "Start of Portal Mover Tasks"
    ...
    $globalBlacklistedFields = $json | select -expand globalBlacklistedFields
    $entityBlackListedFields = $json | select -expand entityBlackListedFields
    $entityFilterXml = $json | select -expand entityFilterXml
    ...
    $EntityRecords = GetEntityRecords $SourceCrmConnection $entities $SourceMetadata $TargetMetadata $globalBlacklistedFields $entityBlackListedFields $entityFilterXml
...
Catch {
    Write-Error -Message ("Error in Portal Mover script: " + $_.Exception.Message)
}